...
namespace WikiHero.Services
{
    public interface IApiComicsVine 
    {
        ...
        Task<ResultCharacters> GetMoreCharacter(string api_key, int offset,string publisher);
        ...
        Task<ResultCharacters> GetCharacter(string api_key, string publisher);
        ...
        Task<ResultCharacters> GetRecentCharacters(string api_key, string publisher);
        ...
        Task<ResultCharacters> SearchCharacters(string name, string api_key, int offset, string publisher);

        [Get("/api/character/4005-{id}?format=json& api_key={api_key}")]
        Task<ResultCharacter> FindCharacter(int id, string api_key, string publisher);
        ...
        [Get("/api/teams?format=json&field_list=id,name,image&api_key={api_key}&filter=id:{filter}&limit=30")]
        Task<ResultTeam> FindTeamsCharacter(string api_key, string filter);

        [Get("/api/characters?format=json&api_key={api_key}&field_list=id,image,name&filter=id:{filter}")]
        Task<ResultCharacters> FindEnenmyCharacter(string api_key, string filter);
        [Get("/api/issues?format=json&api_key={api_key}&field_list=image,name&filter=id:{filter}")]
        Task<ResultComics> FindCharacterComics(string api_key, string filter);
        [Get("/api/volumes?format=json&api_key={api_key}&field_list=image,name&filter=id:{filter}&limit=30")]
        Task<ResultVolume> FindCharactersVolumes(string api_key, string filter);

        [Get("/api/movies?format=json&api_key={api_key}&field_list=image,name&filter=id:{filter}")]
        Task<ResultMovies> FindCharactersMovies(string api_key, string filter);
